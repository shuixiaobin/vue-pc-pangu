 ...
 */
Util.FORM.isLegalTel = (telNumber, reg = /^[1](3|4|5|6|7|8|9)[0-9]{9}$/) => {
...
Util.formatTime2 = function(date) {
  var year = date.getFullYear()
  var month = date.getMonth() + 1
  var day = date.getDate()
...
Util.dateFormatter = function (val, template = 'yyyy-MM-dd hh:mm:ss') {
  ...
  return template
    .replace(/yyyy/gi, y)
    .replace(/yy/gi, (y + '').substr(2,2))
    .replace(/MM/g, m <= 9 ? '0' + m : m)
    .replace(/M/g, m)
    .replace(/dd/gi, d <= 9 ? '0' + d : d)
    .replace(/d/gi, d)
    .replace(/hh/gi, h <= 9 ? '0' + h : h)
    .replace(/h/gi, h)
    .replace(/mm/gi, min <= 9 ? '0' + min : min)
    .replace(/m/gi, min)
    .replace(/ss/gi, s <= 9 ? '0' + s : s)
    .replace(/s/gi, s)
}
/**
...
Util.timeStampDate = function(date) {
  if (!date) {
    return ''
  }
...
Util.setCookie = (c_name, value, day) =>{
  const exdate = new Date();
  exdate.setDate(exdate.getDate() + day);

  var currentCookie = `${c_name}=${value};expires=${exdate.toGMTString()};path=/;`;
  if(process.env.NODE_ENV == 'production'){
    var doman = WEBPACK_BUILD_TARGET === 'production' ? '.htexam.com' : '.huatu.com';
    currentCookie = currentCookie += `domain=${doman}`;
  }
...
Util.getMenuList = function getMenuListByRoutes(routes, accessMap) {
  ...
  routes.forEach(item => {
    ...
    if(item.meta && item.meta.hideInMenu){
      ...
      if (Util.hasChild(item) && (item.meta && item.meta.title)) {//非叶子节点
        item.show = getMenuListByRoutes(item.children, accessMap);
        if (item.show) {
          listShow = true;
        }
      }else if(!item.meta || (item.meta && !item.meta.access)) {// meta未设置或meta.accesse未设置
          item.show =false; 
      }else if(item.meta && item.meta.access){ //需要判断权限的菜单
				if([].concat(accessMap).includes(item.meta.access)){
          item.show = true;
          listShow = true;
        }else{
          item.show = false;
...
Util.getPathByAcess = (routes, acess) => {
  var path="";
  findDataIds(routes);
  return path;
  function findDataIds(ary) {
    for(var i in ary){
      if(ary[i].meta && ary[i].meta.access){
        if(ary[i].meta.access == acess) {
          path =ary[i].path;
          return;
        }
      }
      if(ary[i].children){
        findDataIds(ary[i].children);
      }
    }
  }
}

Util.getMenuTitile = (routes) => {
  var obj,result={}
  findDataIds(routes);
  return result;
  function findDataIds(ary) {
    for(var i in ary){
      if(ary[i].menuType == 1 && ary[i].frontToolName){
          obj={
            title:ary[i].menuName,
            id:ary[i].id,
          }
          result[ary[i].frontToolName]=obj;
      }
      if(ary[i].children){
        findDataIds(ary[i].children);
      }
...
Util.stringStripper = stringStripper

Util.CleanPastedHTML = function (input) {
...
/* 比较两个数组，返回[增项，减项] */
Util.compareAry=function (oldArr, newArr) {
  var t = this;
  ...
  var a = oldArr.unique4(); //旧数组
  var d = oldArr.unique4(); //旧数组
  var b = newArr.unique4();//新数组
  var e = newArr.unique4();//新数组
...
export function assert (boolean, msg) {
  if (boolean) return
  throw new Error(msg)
}

// 传入字符串 或 数字，返回金钱格式，  例如   123.00
Util.moneyFormat = (number) => {
	if(number || number == 0){
		let lastLength = 0;
		if(number.toString().indexOf('.') >= 0){
			lastLength = number.toString().indexOf('.') + 3
		}else {
			lastLength = number.length;
		}
		const strNumber = number.toString().substring(0, lastLength)

...
<script>
  import { addAutoCollectList, addPartnerClass } from '@/api/thirdParty'
  export default {
    name: 'onlineClassDialog',
    props: {
      userName: {
        type: String,
        default: ''
    ...
    created() {
      this.getOnlineClassList()
    },
    methods: {
      // 集合列表
      async getOnlineClassList() {
        let param = {
          cateId: this.currentCateId, // 考试类型 id
          collectId: this.currentClassId, // 搜索集合 id
          collectTitle: this.currentClassName, // 搜索集合名称
          page: this.page.class_page
        }
        let res = await addAutoCollectList(param)
      ...
      // 选中/反选
      isClassChecked(item) {
        let checkClassListIndex = this.checkClassList.data.findIndex(checked => checked.id == item.id)
      ...
      offDialog() {
        this.page.class_page = 1
        this.$emit('update:autoCollectionDialogVisible', false)
      }
    },
    watch: {

...
<script>
	...
	export default{
		...
		watch: {
			visible(newVal){
				if(newVal){
					this.piceureSrc = this.protoObject.exam_card_code_pic; 
				}
			}
		},
		methods: {
			...
			async handleUpdate(){
				if(!this.protoObject.protocol_student_id){
					this.$message.error('【暂无协议用户信息，请督促学员签署协议】')
					return;
				}
				this.protoObject.exam_card_code_pic = this.piceureSrc;
				const params = {
					protocolStudentId: this.protoObject.protocol_student_id,
					stuInfo: this.protoObject.exam_card_code_pic,
					actionType: '2',

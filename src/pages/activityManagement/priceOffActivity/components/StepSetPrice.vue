...
export default {
  ...
  methods: {
    ...
    setPriceBatch() {
      ...
      if (this.batchActivityPrice) {
        this.netClassArr.forEach(item => {
          let offPrice = (item.price - activityPrice).toFixed(2);
          this.$set(item, 'activityPrice', activityPrice);
          this.$set(item, 'OffPrice', offPrice);
    ...
    onPriceBlur(item) {
      ...
      // 计算直降价格
      let offPrice = item.price - item.activityPrice;
      this.$set(item, 'OffPrice', offPrice.toFixed(2));
      // 计算赠送积分
      this.$set(item, 'point', Math.floor(item.activityPrice / 10));
    },
    validate() {},
    ...
    onForward() {
      return new Promise((resovle, reject) => {
        ...
        if (
          ...
          reject();
        } else if (this.netClassArr.find(item => item.activityPrice < 0)) {
          this.$message.error('课程直降价格不能大于课程价格');
          reject();
        } else if (
          this.netClassArr.find(
            item =>
          ...
          reject();
        } else if (this.netClassArr.find(item => item.OffPrice <= 0)) {
          this.$message.error('课程活动价格不能高于原价');
          reject();
        } else if (
          this.netClassArr.find(
            item =>

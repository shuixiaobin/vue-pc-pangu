export default class PriceOffActivity {
    ...
    if (activity.netClassArr && Array.isArray(activity.netClassArr)) {
      paramObj.netClassArr = activity.netClassArr.map(item => {
        return {
          ...
          OffPrice: parseFloat(item.OffPrice),
          maxNum: item.maxNum,
          point: item.point
        }
      })
  ...
  // 根据活动详情接口数据 生成活动对象; user用来在复制活动时变更创建人
  static makeupActivity ({activity, netClass}, user) {
    ...
    return Object.assign(new PriceOffActivity(), {

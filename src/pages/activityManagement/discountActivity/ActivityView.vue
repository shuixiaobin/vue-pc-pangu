...
import { getDiscountActivityDetail } from '@/api/activityManage';
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.clearActivity();
      this.loadActivity({
        loadFun: () => getDiscountActivityDetail({ activityId: this.activityId }),
        makeUp: DiscountActivity.makeupActivity,
      })
        .then(() => {
          this.loading = false;
        })

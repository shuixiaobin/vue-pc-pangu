(function() {
        ...
        Base = {
            ...
            getAllPic: function(sel, $w, editor) {
                ...
                $.each($imgs, function(index, node) {
                    ...
                    return arr.push({
                        src: node.src,
                        width: imgWidth ? imgWidth : node.width,
                        height: imgHeight ? imgHeight : node.height
                    });
                });
            ...
            $(me.dialog).off('change.sss').on("change.sss", ".edui-image-file", function(e) {
                ...
                $.each(fileList, function(i, f) {
                    if (/^image/.test(f.type)) {
                        var url=`${sessionStorage.getItem("WEBPACK_AJAX_BASE_URL")}/pand/file/upload`
    ...
    var setSize = {
        ...
        initEvt: function(dom) {
            ...
            var w = dom.width || 0;
            var h = dom.height || 0;
            $('#width').off("input propertychange").on("blur", function() {
                ...
                if (!$(this).val()) return;
                imgWidth = $(this).val();
                imgHeight = parseInt($(this).val() / ratio);
                $('#height').val(parseInt($(this).val() / ratio));

            });

            $('#height').off("input propertychange").on("blur", function() {
                // if ($('#lock').is(':checked')) {
                ...
                if (!$(this).val()) return;
                imgHeight = $(this).val();
                imgWidth = parseInt($(this).val() * ratio);
                $('#width').val(parseInt($(this).val() * ratio));
            });
    ...
    UM.registerWidget('image', {
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=image_url%>image.css\">" +
            ...
            '<div class="container"><div class="row" style="padding-top:15px;">' +
            '<label>说明：输入宽/高来等比例设置图片大小。</label>' +
            '<div class="input-group"><span style="padding-right:10px;">宽度</span><input class="form-control" type="text" id="width" placeholder="单位px"  style="margin:15px 0;"></div>' +
            '<div class="input-group"><span style="padding-right:10px;">高度</span><input class="form-control" type="text" id="height" placeholder="单位px"  style="margin:15px 0;"></div>' +
            "</div></div>" +
